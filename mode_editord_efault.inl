...
void DefaultEditorMode::InitGUI() {
  ...
  selectorRect.setTexture(tileMap->TileSheet());
  selectorRect.setTextureRect(textureRect);

  textureSelector = new gui::TextureSelector(
      20.f, 20.f, 1000.f, 500.f, state_data_->grid_size, tileMap->TileSheet(),
...
DefaultEditorMode::DefaultEditorMode(StateData* state_data, TileMap* tiles,
    ...
    : EditorMode(state_data, tiles, editor_state_data) {
  InitVariables();
  InitGUI();
}
...
void DefaultEditorMode::UpdateInput(const float& dt) {
  ...
  if (sf::Mouse::IsButtonPressed(sf::Mouse::Left) && KeyTime()) {
    if (!sidebar.GlobalBounds().contains(
            sf::Vector2f(*editor_state_data_->mousePosWindow))) {
      if (!textureSelector->getActive()) {
        if (tileAddLock) {
          if (tileMap->TileEmpty(editor_state_data_->mousePosGrid->x,
                                 editor_state_data_->mousePosGrid->y, 0)) {
            tileMap->addTile(editor_state_data_->mousePosGrid->x,
                             editor_state_data_->mousePosGrid->y, 0,
                             textureRect, collision, type);
          }
        } else {
          tileMap->addTile(editor_state_data_->mousePosGrid->x,
                           editor_state_data_->mousePosGrid->y, 0, textureRect,
                           collision, type);
        }
      } else {
  ...
  // Remove a tile from the tilemap
    ...
    if (!sidebar.GlobalBounds().contains(
            sf::Vector2f(*editor_state_data_->mousePosWindow))) {
      if (!textureSelector->getActive())
        tileMap->removeTile(editor_state_data_->mousePosGrid->x,
                            editor_state_data_->mousePosGrid->y, 0);
    }
  }
...
void DefaultEditorMode::UpdateGUI(const float& dt) {
  textureSelector->Update(*editor_state_data_->mousePosWindow, dt);

  if (!textureSelector->getActive()) {
    selectorRect.setTextureRect(textureRect);
    selectorRect.SetPosition(
        editor_state_data_->mousePosGrid->x * state_data_->grid_size,
        editor_state_data_->mousePosGrid->y * state_data_->grid_size);
  }
  ...
  ss << editor_state_data_->mouse_pos_value_->x << " "
     << editor_state_data_->mouse_pos_value_->y << "\n"
     << editor_state_data_->mousePosGrid->x << " "
     << editor_state_data_->mousePosGrid->y << "\n"
     << textureRect.left << " " << textureRect.top << "\n"
     << "Collision: " << collision << "\n"
     << "Type: " << type << "\n"
     << "Tiles: "
     << tileMap->LayerSize(editor_state_data_->mousePosGrid->x,
                           editor_state_data_->mousePosGrid->y, layer)
     << "\n"
